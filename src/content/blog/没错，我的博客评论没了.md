...
> 💡 **关于存储**：Waline 支持多种数据库，包括 MongoDB、PostgreSQL、MySQL、SQLite、LeanCloud 等。我这次用的是 LeanCloud（国内版），免费额度够用，而且不用自己维护 MongoDB。
...
|------|------|------|
| 部署平台 | Netlify | Vercel |
| 数据库 | MongoDB（忘了） | LeanCloud |
| 邮件通知 | ❌ 挂了 | ✅ 正常 |
| 加载速度 | 🐢 特别慢 | 🚀 飞快 |
| 历史评论 | 😭 没了 | 🆕 重新开始 |

虽然评论没了有点心疼，但重新部署后一切正常，邮件通知也恢复了，加载速度肉眼可见地提升。Vercel + LeanCloud 的组合对于个人博客来说完全够用，而且免费。
